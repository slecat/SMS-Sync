...
const groupId = 'Dulin';
const deviceId = 'desktop_smoke_sms2_' + Date.now();
const ws = new WebSocket(url);
let done = false;
const finish = (code, msg) => { if (!done) { done = true; if (msg) console.log(msg); process.exit(code); } };
ws.on('open', () => {
  ws.send(JSON.stringify({ type: 'register', deviceId, deviceName: 'DesktopSmokeSms2', groupId }));

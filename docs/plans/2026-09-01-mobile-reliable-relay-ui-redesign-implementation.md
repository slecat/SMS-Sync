...
- 已完成：设置页原生队列诊断桥接、已确认 Outbox 的 7 天清理策略；服务端 replay 已支持最近 100 条短信。
- 已完成：服务端断线重连端到端夹具，覆盖桌面离线补拉、消息去重和 delivery-ack 闭环；本地与远端均通过。
- 已完成：真机发送测试链路核对；临时 WebSocket 的来源身份兜底已修复，测试消息不再回发到手机自身，远端事件已确认仅转发到桌面端。
- 待真机验收：在 Android 16 实机执行锁屏、断网、系统回收、重启、升级和通知监听重绑的长时间保活压测；当前已确认前台服务可持续运行，尚未完成真实入站短信与长时间压测。
